...
//copied : https://min-wachya.tistory.com/171
class SwipeHelper(adapter: CategoryAdapter) :
    ItemTouchHelper.Callback() {  // ItemTouchHelper.Callback 을 구현해야 한다
    ...
    override fun clearView(recyclerView: RecyclerView, viewHolder: RecyclerView.ViewHolder) {
        ...
        previousPosition =
            viewHolder.absoluteAdapterPosition       // 드래그 또는 스와이프 동작이 끝난 view의 position 기억하기
    ...
    override fun onSelectedChanged(viewHolder: RecyclerView.ViewHolder?, actionState: Int) {
        viewHolder?.let {
            currentPosition =
                viewHolder.absoluteAdapterPosition    // 현재 드래그 또는 스와이프 중인 view 의 position 기억하기
    ...
    private fun getView(viewHolder: RecyclerView.ViewHolder): View =
        viewHolder.itemView.findViewById(R.id.category_swipe_view)
    ...
    fun removePreviousClampAnyway(recyclerView: RecyclerView) {
        previousPosition?.let {
            val viewHolder = recyclerView.findViewHolderForAdapterPosition(it) ?: return
            getView(viewHolder).animate().x(0f).setDuration(100L).start()
            setTag(viewHolder, false)
